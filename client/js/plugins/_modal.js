...
export default {
	...
	init: function ($el, opt, exportObj) {
		let _show = (exportObj.show = function () {
			...
			emit('modal.hidden.except', [opt._pid]);
			opt.beforeShow && trigger(opt.beforeShow, $el, opt, exportObj);
			document.querySelector('body').classList.add('modal-model');
			opt.root.style.display = 'block';
			setTimeout(function () {
				opt.root.classList.add('modal-active');
				opt.afterShow && trigger(opt.afterShow, $el, opt, exportObj);
			}, 50);
			on('modal.hidden.except' + opt._pid, function (e, id) {
				if (id != opt._pid) {
					exportObj.hide();
				}
			});
		...
		let _hide = (exportObj.hide = function () {
			if (opt.root) {
				...
				opt.root.classList.remove('modal-active');
				setTimeout(function () {
					opt.root.style.display = 'none';
					document.querySelector('body').classList.remove('modal-model');
					opt.afterHide && trigger(opt.beforeHide, $el, opt, exportObj);
					if (!opt.cache) {
						opt.root.remove();
						opt.root = null;
						off('modal.hidden.except' + opt._pid, function (e, id) {
							if (id != opt._pid) {
								exportObj.hide();
							}
						});
		...
		let _render = (exportObj.render = function () {
			let template = `<div id="${opt._pid}" class="modal modal-${opt.theme}" tabIndex="-1"><div class="modal-overlay"></div><div class="modal-panel"><div class="modal-body"><button class="modal-title-close" data-modal="close" type="button"><i class="icon-close"></i></button>${opt.html}</div></div>`;
			emit('dom.load');
			...
			opt.root.querySelectorAll('[data-modal="close"]').forEach((item) => {
			...
			if (opt.autoclose) {
				opt.root.querySelector('.modal-overlay').addEventListener('click', function () {
					_hide();
				});
			}
