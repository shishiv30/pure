...
export default {
	...
	init: function ($el, opt, exportObj) {
        let shifter = $el;
        let scroller = select('[data-role="scroller"]', shifter);
        ...
        select('.arrow.next', shifter).addEventListener('click', exportObj.next);
        select('.arrow.prev', shifter).addEventListener('click', exportObj.prev);
	}
};
